...
options:
  ...
  availability_requirements:
    ...
    suboptions:
      ...
      type:
        ...
        - When creating a new virtual, if this value is not specified, the default
          of 'all' will be used.
        type: str
    type: dict
    version_added: 2.6
  limits:
    ...
    - When you enable one or more limit settings, the system then uses that data to
      take servers in and out of service.
    - You can define limits for any or all of the limit settings. However, when a
      server does not meet the resource threshold limit requirement, the system marks
      the entire server as unavailable and directs load-balancing traffic to another
      resource.
    ...
    suboptions:
      ...
      connections_limit:
        ...
        - Specifies the maximum number of concurrent connections, combined, for all
          of the virtual servers on the server.
  ...
  port:
    description:
    - Specifies the service port number for the virtual server or pool member. For
      example, the HTTP service is typically port 80.
    - To specify all ports, use an C(*).
    - When creating a new GTM virtual server, if this parameter is not specified,
      a default of C(*) will be used.
    type: int
  server_name:
    description:
  ...
  virtual_server_dependencies:
    ...
    - If any of the specified servers are unavailable, the current virtual server
