...
notes:
- Due to ID685681 it is not possible to execute ng_* tools via REST api on v12.x and
  13.x, once this is fixed this restriction will be removed.
...
options:
  ...
  force:
    default: true
    description:
    - If C(no), the file will only be transferred if it does not exist in the the
      destination.
    type: bool
  name:
    description:
    - The name of the APM policy or APM access profile exported to create a file on
      the remote device for downloading.
    required: true
    type: str
  partition:
    default: Common
