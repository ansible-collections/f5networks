...
options:
  accept_rcode:
    ...
    - When C(anything), specifies that the status of the node will be marked up irrespective
      of the RCODE in the DNS message received.
    - If this parameter is set to C(anything), it will disregard the C(receive) string,
      and nullify it if the monitor is being updated.
    type: str
  adaptive:
    description:
    ...
    - When C(yes), the monitor determines the state of a service based on how divergent
      from the mean latency a monitor probe for that service is allowed to be. Also,
      values for the C(allowed_divergence), C(adaptive_limit), and and C(sampling_timespan)
      will be enforced.
  ...
  adaptive_limit:
    description:
    - Specifies the absolute number of milliseconds that may not be exceeded by a
      monitor probe, regardless of C(allowed_divergence) setting, for a probe to be
  ...
  allowed_divergence_value:
    description:
    - When specifying a new monitor, if C(adaptive) is C(yes), and C(type) is C(relative),
  ...
  answer_section_contains:
    ...
    - When creating a new monitor, if this value is not specified, the default value
      is C(query-type).
    ...
    - When C(any-type), specifies that the DNS message should contain at least one
      answer.
    - When C(anything), specifies that an empty answer is enough to mark the status
      of the node up.
    type: str
  description:
    description:
  ...
  manual_resume:
    ...
    - If you set this option to C(yes), you must manually re-enable the resource before
  ...
  parent:
    ...
    - The parent template of this monitor template. Once this value has been set,
      it cannot be changed. By default, this value is the C(dns) parent on the C(Common)
      partition.
    type: str
  partition:
    default: Common
  ...
  reverse:
    ...
    - When the monitor is in reverse mode, a successful receive string match marks
      the monitored object down instead of up. You can use the this mode only if you
      configure the C(receive) option.
  ...
  time_until_up:
    ...
    - Specifies the amount of time in seconds after the first successful response
      before a node will be marked up.
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.ipaddress import validate_ip_address
