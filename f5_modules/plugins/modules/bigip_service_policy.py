...
description:
- Service policies allow you to configure timers and port misuse rules, if enabled,
  on a per rule or per context basis.
...
options:
  ...
  port_misuse_policy:
    ...
    - Requires that C(afm) be provisioned to use. If C(afm) is not provisioned, this
