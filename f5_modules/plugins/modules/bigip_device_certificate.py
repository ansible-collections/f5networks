...
options:
  add_to_trusted:
    default: false
    description:
    - Specified if the certificate should be added to the trusted client and server
      certificate files.
    type: bool
  cert_name:
    default: server.crt
    ...
    - If the name is not default C(server.crt), the module will configure C(httpd)
      to use them prior to restarting the C(httpd) daemon.
    type: str
  days_valid:
    description:
  ...
  force:
    default: false
    description:
    - When C(yes), will update or overwrite the existing certificate when it is not
      expired device.
    - When C(no), the certificate will only be updated/overwritten if expired.
    - Generally should be C(yes) only in cases where you need to update certificate
      that is about to expire.
    - This option is also needed when generating new certificate to replace non expired
      one.
    type: bool
  issuer:
    description:
  ...
  key_name:
    ...
    - If the name is not default C(server.key), the module will configure C(httpd)
      to use them prior to restarting the C(httpd) daemon.
    type: str
  key_size:
    choices:
