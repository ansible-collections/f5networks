...
description:
- Manage DNS zones on BIG-IP. The zones managed here are primarily used for configuring
  DNS Express on BIG-IP. This module does not configure zones that are found in BIG-IP
  ZoneRunner.
...
options:
  dns_express:
    ...
    suboptions:
      ...
      notify_action:
        ...
        - When C(bypass), the NOTIFY message does not go to DNS Express, but instead
          goes to a back-end DNS server (subject to the value of the Unhandled Query
          Action configured in the DNS profile applied to the listener that handles
          the DNS request).
  ...
  name:
    ...
    - The name must begin with a letter and contain only letters, numbers, and the
