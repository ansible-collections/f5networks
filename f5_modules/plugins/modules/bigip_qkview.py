...
- Tim Rupp (@caphrim007)
description:
- Manages creating and downloading qkviews from a BIG-IP. Various options can be provided
  when creating qkviews. The qkview is important when dealing with F5 support. It
  may be required that you upload this qkview to the supported channels during resolution
  of an SRs that you may have opened.
...
- If you are using this module with either Ansible Tower or Ansible AWX, you should
  ...
  More informat can be found here https://clouddocs.f5.com/products/orchestration/ansible/devel/usage/module-usage-with-tower.html
options:
  asm_request_log:
    default: false
    description:
    - When C(True), includes the ASM request log data. When C(False), excludes the
      ASM request log data.
    type: bool
  ...
  max_file_size:
    default: 0
    description:
    - Max file size, in bytes, of the qkview to create. By default, no max file size
      is specified.
    type: int
