...
description:
- Manages AFM logging profiles configured in the system along with basic information
  about each profile.
...
options:
  ...
  ip_intelligence:
    ...
    suboptions:
      ...
      log_translation_fields:
        description:
        - This option is used to enable or disable the logging of translated (i.e
          server side) fields in IP Intelligence log messages.
      ...
      rate_limit:
        ...
        - Defines a rate limit for all combined IP intelligence log messages per second.
          Beyond this rate limit, log messages are not logged until the threshold
          drops below the specified rate.
  ...
  port_misuse:
    ...
    suboptions:
      ...
      rate_limit:
        description:
        - Defines a rate limit for all combined port misuse log messages per second.
          Beyond this rate limit, log messages are not logged until the threshold
          drops below the specified rate.
        - To specify an indefinite rate, use the value C(indefinite).
        - If specifying a numeric rate, the value must be between C(1) and C(4294967295).
  ...
  state:
    ...
    - When C(state) is C(absent), ensures that resource is removed. Attempts to remove
