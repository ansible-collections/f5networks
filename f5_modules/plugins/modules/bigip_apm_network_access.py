...
options:
  allow_local_dns:
    description:
    - Enables local access to DNS servers configured on client prior to establishing
      network access connection.
    type: bool
  allow_local_subnet:
    description:
    - Enables local subnet access and local access to any host or subnet in routes
      specified in the client routing table.
  ...
  dtls:
    description:
    - When C(yes) the network access connection uses Datagram Transport Level Security
      instead of TCP, to provide better throughput for high demand applications like
      VoIP or streaming video.
    type: bool
  dtls_port:
    description:
    - Specifies the port number that the network access resource uses for secure UDP
      traffic with DTLS.
    type: int
  excluded_dns_addresses:
    description:
    - Specifies the DNS address spaces for which traffic is not forced through the
      tunnel.
    type: list
  ...
  ipv4_address_space:
    ...
    - This option is mandatory when creating a new resource and C(split_tunnel) is
      set to C(yes).
    suboptions:
      subnet:
        description:
  ...
  ipv4_lease_pool:
    ...
    - When lease pool is referenced in full path format, the C(partition) parameter
      is ignored.
    type: str
  ipv6_address_space:
    description:
  ...
  split_tunnel:
    description:
    - Specifies that only the traffic targeted to a specified address space is sent
      over the network access tunnel.
    type: bool
  state:
    choices:
    - present
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.ipaddress import ip_network
