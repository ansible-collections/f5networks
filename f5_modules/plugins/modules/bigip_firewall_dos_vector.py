...
options:
  allow_advertisement:
    description:
    - Specifies that addresses that are identified for blacklisting are advertised
      to BGP routers
    type: bool
  attack_ceiling:
    description:
  ...
  name:
    ...
    - When C(dns-malformed), configures the "dns-malformed" DNS Protocol Security
      vector.
    ...
    - When C(register), configures the "SIP REGISTER Method" SIP Protocol Security
      vector.
    - When C(sip-malformed), configures the "sip-malformed" SIP Protocol Security
      vector.
    - When C(subscribe), configures the "SIP SUBSCRIBE Method" SIP Protocol Security
      vector.
  ...
  state:
    ...
    - When C(state) is C(detect-only), ensures that the vector does not enforce limits
      and thresholds (rate limiting, dopping, etc), but is still tracked in logs and
      statistics.
    ...
    - When C(state) is C(learn-only), ensures that the vector does not "detect" any
      attacks. Only learning and stat collecting is performed.
    required: true
    type: str
  threshold_mode:
    choices:
