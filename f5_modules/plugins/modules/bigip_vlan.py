...
options:
  cmp_hash:
    ...
    - src
    description:
    - Specifies how the traffic on the VLAN will be disaggregated. The value selected
      determines the traffic disaggregation method. You can choose to disaggregate
      traffic based on C(source-address) (the source IP address), C(destination-address)
      (destination IP address), or C(default), which specifies that the default CMP
      hash uses L4 ports.
    - When creating a new VLAN, if this parameter is not specified, the default of
      C(default) is used.
    type: str
    version_added: 2.5
  dag_round_robin:
    ...
    - Specifies whether some of the stateless traffic on the VLAN should be disaggregated
      in a round-robin order instead of using a static hash. The stateless traffic
      includes non-IP L2 traffic, ICMP, some UDP protocols, and so on.
    - When creating a new VLAN, if this parameter is not specified, the default of
      (no) is used.
    type: bool
    version_added: 2.5
  dag_tunnel:
    ...
    - Specifies how the disaggregator (DAG) distributes received tunnel-encapsulated
      ...
      in inner headers. Select C(outer) to distribute packets based on information
      in outer headers without inspecting inner headers.
    - When creating a new VLAN, if this parameter is not specified, the default of
      C(outer) is used.
  ...
  fail_safe:
    description:
    - When C(yes), specifies that the VLAN takes the specified C(fail_safe_action)
      if the system detects a loss of traffic on this VLAN's interfaces.
    type: bool
    version_added: 2.8
  fail_safe_action:
    choices:
    ...
    - failover
    description:
    - Specifies the action that the system takes when it does not detect any traffic
      on this VLAN, and the C(fail_safe_timeout) has expired.
    type: str
    version_added: 2.8
  fail_safe_timeout:
    description:
  ...
  interfaces:
    description:
    - Interfaces that you want added to the VLAN. This can include both tagged and
      untagged interfaces as the C(tagging) parameter specifies.
  ...
  mtu:
    description:
    - Specifies the maximum transmission unit (MTU) for traffic on this VLAN. When
      creating a new VLAN, if this parameter is not specified, the default value used
      will be C(1500).
  ...
  name:
    description:
    - The VLAN to manage. If the special VLAN C(ALL) is specified with the C(state)
      value of C(absent) then all VLANs will be removed.
    required: true
    type: str
  partition:
    default: Common
  ...
  source_check:
    description:
    - When C(yes), specifies that the system verifies that the return route to an
      initial packet is the same VLAN from which the packet originated.
    - The system performs this verification only if the C(auto_last_hop) option is
      C(no).
    type: bool
    version_added: 2.8
  state:
    ...
    default: present
    description:
    - The state of the VLAN on the system. When C(present), guarantees that the VLAN
      exists with the provided attributes. When C(absent), removes the VLAN from the
      system.
    type: str
  tag:
    description:
  ...
  tagged_interfaces:
    ...
    - tagged_interface
    description:
    - Specifies a list of tagged interfaces and trunks that you want to configure
      for the VLAN. Use tagged interfaces or trunks when you want to assign a single
      interface or trunk to multiple VLANs.
  ...
  untagged_interfaces:
    ...
    - untagged_interface
    description:
    - Specifies a list of untagged interfaces and trunks that you want to configure
      for the VLAN.
