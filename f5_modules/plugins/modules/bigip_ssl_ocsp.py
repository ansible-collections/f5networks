...
options:
  ...
  clock_skew:
    description:
    - Specifies the tolerable absolute difference in the clocks of the responder and
      the BIG-IP system, in seconds.
    type: int
  connection_timeout:
    description:
    - Specifies the time interval that the BIG-IP system waits for before ending the
      connection to the OCSP responder, in seconds.
    type: int
  connections_limit:
    description:
    - Specifies the maximum number of connections per second allowed for the OCSP
      certificate validator.
    type: int
  dns_resolver:
    description:
    - Specifies the internal DNS resolver the BIG-IP system uses to fetch the OCSP
      response.
    ...
    - Use this option when either there is a DNS server that can do the name-resolution
      of the OCSP responders or the OCSP responder can be reached on one of BIG-IP
      system's interfaces.
    type: str
  hash_algorithm:
    choices:
  ...
  responder_url:
    description:
    - Specifies the absolute URL that overrides the OCSP responder URL obtained from
      the certificate's AIA extensions. This should be an HTTP-based URL.
    type: str
  route_domain:
    description:
    - Specifies the route domain for fetching an OCSP response using HTTP forward
      proxy.
    type: str
  state:
    choices:
  ...
  status_age:
    description:
    - Specifies the maximum allowed lag time that the BIG-IP system accepts for the
      'thisUpdate' time in the OCSP response.
    type: int
  strict_responder_checking:
    description:
    - Specifies whether the responder's certificate is checked for an OCSP signing
      extension.
    type: bool
  trusted_responders:
    description:
  ...
  update_password:
    ...
    - C(always) will allow to update passwords if the user chooses to do so. C(on_create)
      will only set the password for newly created OCSP validators.
    type: str
