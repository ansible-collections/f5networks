...
options:
  ...
  phase1_verify_peer_cert:
    ...
    - In IKEv1, specifies whether the identifier sent by the peer is verified with
      ...
      that the entire certificate subject name is compared with the identifier. Address,
      FQDN, or User FQDN; specifies that the certificate's subjectAltName is compared
      with the identifier. If the two do not match, the negotiation fails.
    - When creating a new IKE peer, if this value is not specified, and C(phase1_auth_method)
      is C(rsa-signature), the default is C(no).
    - This parameter is invalid when C(phase1_auth_method) is C(pre-shared-key).
    type: bool
  presented_id_type:
    choices:
    ...
    - override
    description:
    - Specifies the identifier type that the local system uses to identify itself
      to the peer during IKE Phase 1 negotiations.
    type: str
  presented_id_value:
    description:
  ...
  remote_address:
    description:
    - Displays the IP address of the BIG-IP system that is remote to the system you
      are configuring.
    type: str
  state:
    choices:
  ...
  update_password:
    ...
    - C(always) will allow to update passwords if the user chooses to do so. C(on_create)
      will only set the password for newly created IKE peers.
    type: str
  verified_id_type:
    choices:
    - address
    ...
    - override
    description:
    - Specifies the identifier type that the local system uses to identify the peer
      during IKE Phase 1 negotiation.
    - This is a required value when C(version) includes (Cv2).
    - When C(user-fqdn), value of C(verified_id_value) must be in the form of User
      @ DNS domain string.
    type: str
  verified_id_value:
    description:
  ...
  version:
    ...
    - If the system you are configuring is the IPsec initiator, and you select both
      versions, the system tries using IKEv2 for negotiation. If the remote peer does
      not support IKEv2, the IPsec tunnel fails. To use IKEv1 in this case, you must
      deselect Version 2 and try again.
    - If the system you are configuring is the IPsec responder, and you select both
      versions, the IPsec initiator system determines which IKE version to use.
