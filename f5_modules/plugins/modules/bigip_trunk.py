...
options:
  ...
  frame_distribution_hash:
    ...
    - When C(source-destination-mac), specifies that the system bases the hash on
  ...
  lacp_mode:
    ...
    - passive
    description:
    - Specifies the operation mode for link aggregation control protocol (LACP), if
      LACP is enabled for the trunk.
    - When creating a new trunk, if this parameter is not specified, the default is
      C(active).
    ...
    - When C(passive), specifies that the system sends control packets only when the
      partner system has issued a request.
    type: str
  lacp_timeout:
    choices:
    - long
    ...
    - When creating a new trunk, if this parameter is not specified, the default is
      C(long).
    - When C(long), specifies that the system sends an LACP control packet every 30
      seconds.
    - When C(short), specifies that the system sends an LACP control packet every
      1 seconds.
    type: str
  link_selection_policy:
    ...
    - When C(auto), specifies that the system automatically determines which interfaces
      can handle new traffic. For the C(auto) option, the member links must all be
      the same media type and speed.
