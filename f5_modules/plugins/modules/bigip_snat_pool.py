...
notes:
- When C(bigip_snat_pool) object is removed it also removes any associated C(bigip_snat_translation)
  objects.
- This is a BIG-IP behavior not module behavior and it only occurs when the C(bigip_snat_translation)
  objects are also not referenced by another C(bigip_snat_pool).
options:
  description:
    description:
    - A general description of the SNAT pool, provided by the user for their benefit.
      It is optional.
    type: str
    version_added: 2.9
  members:
    ...
    - member
    description:
    - List of members to put in the SNAT pool. When a C(state) of present is provided,
