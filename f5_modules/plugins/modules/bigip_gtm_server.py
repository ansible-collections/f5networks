...
options:
  availability_requirements:
    ...
    suboptions:
      at_least:
        ...
        - This parameter will be ignored if a type of either C(all) or C(require)
          is used.
        type: int
      number_of_probers:
        description:
        - Specifies the number of probers that should be used when running probes.
        - When creating a new virtual server, if this parameter is specified, then
          the C(number_of_probes) parameter must also be specified.
        - The value of this parameter should always be B(higher) than, or B(equal
          to), the value of C(number_of_probers).
        - This parameter is only relevant when a C(type) of C(require) is used.
        - This parameter will be ignored if a type of either C(all) or C(at_least)
          is used.
        type: int
      number_of_probes:
        description:
        - Specifies the minimum number of probes that must succeed for this server
          to be declared up.
        - When creating a new virtual server, if this parameter is specified, then
          the C(number_of_probers) parameter must also be specified.
  ...
  devices:
    description:
    - Lists the self IP addresses and translations for each device. When creating
      a new GTM server, this value is required. This list is a complex list that specifies
      a number of keys.
    ...
    - Specifying duplicate C(name) fields is a supported means of providing device
      addresses. In this scenario, the addresses will be assigned to the C(name)'s
      list of addresses.
    type: list
  iquery_options:
    description:
    ...
    suboptions:
      ...
      allow_snmp:
        description:
        - Specifies that the system checks the performance of a server running an
          SNMP agent.
        type: bool
    type: dict
    version_added: 2.7
  limits:
    ...
    suboptions:
      ...
      cpu_limit:
        ...
        - If percent of CPU usage goes above the limit, the system marks the server
          as unavailable.
        type: int
      memory_enabled:
        description:
      ...
      memory_limit:
        ...
        - If available memory falls below this limit, the system marks the server
          as unavailable.
        type: int
      packets_enabled:
        description:
  ...
  link_discovery:
    ...
    - Specifies whether the system auto-discovers the links for this server. When
      creating a new GTM server, if this parameter is not specified, the default value
      C(disabled) is used.
    - If you set this parameter to C(enabled) or C(enabled-no-delete), you must also
      ensure that the C(virtual_server_discovery) parameter is also set to C(enabled)
      or C(enabled-no-delete).
    type: str
  monitors:
    description:
    - Specifies the health monitors that the system currently uses to monitor this
      resource.
  ...
  prober_fallback:
    ...
    - none
    description:
    - Specifies the type of prober to use to monitor this server's resources when
      the preferred prober is not available.
    ...
    - The choices are mutually exclusive with prober_preference parameter, with the
      exception of C(any-available) or C(none) option.
    type: str
    version_added: 2.8
  prober_pool:
    description:
    - Specifies the name of the prober pool to use to monitor this server's resources.
    - From C(TMOS) version C(13.x) and up, this parameter is mandatory when C(prober_preference)
      is set to C(pool).
    - Format of the name can be either be prepended by partition (C(/Common/foo)),
      or specified just as an object name (C(foo)).
    - In C(TMOS) version C(12.x) prober_pool can be set to empty string to revert
      to default setting of inherit.
    type: str
    version_added: 2.8
  prober_preference:
    choices:
  ...
  server_type:
    ...
    - windows-2000-server
    description:
    - Specifies the server type. The server type determines the metrics that the system
      can collect from the server. When creating a new GTM server, the default value
      C(bigip) is used.
    type: str
  state:
    choices:
    ...
    - The server state. If C(absent), an attempt to delete the server will be made.
      This will only succeed if this server is not in use by a virtual server. C(present)
      creates the server and enables it. If C(enabled), enable the server if it exists.
      If C(disabled), create the server if needed, and set state to C(disabled).
    type: str
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.common import is_empty_list
