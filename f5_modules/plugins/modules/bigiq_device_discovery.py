...
options:
  access_conflict_policy:
    ...
    - Sets the conflict resolution policy for Access module C(apm) objects, only used
      when C(apm) module is specified.
    type: str
  access_group_first_device:
    default: true
    description:
    - Specifies if the imported device is the first device in the access group to
      import shared configuration for that access group.
    type: bool
  access_group_name:
    description:
  ...
  conflict_policy:
    ...
    - Sets the conflict resolution policy for shared objects across BIG-IP devices,
      except LTM profiles and monitors.
    type: str
  device_address:
    description:
  ...
  modules:
    ...
    - The C(ltm) module must always be specified when performing discovery or re-discovery
      of the the device.
    - When C(asm) or C(afm) are specified C(shared_security) module needs to also
      be declared.
    type: list
  state:
    choices:
  ...
  statistics:
    ...
    suboptions:
      ...
      interval:
        ...
        default: 60
        description:
        - Specify the interval in seconds the data is collected from the discovered
          device.
        type: int
      stat_modules:
        choices:
  ...
  use_bigiq_sync:
    default: false
    description:
    - When set to true, BIG-IQ will manually synchronize configuration changes between
      members in a DSC cluster.
    type: bool
  versioned_conflict_policy:
    ...
    - keep_version
    description:
    - Sets the conflict resolution policy for LTM profile and monitor objects that
      are specific to a BIG-IP software version.
    type: str
