...
notes:
- Support for TMOS versions below v12.x has been deprecated for this module, and will
...
options:
  alternate_lb_method:
    ...
    description:
    - The load balancing mode that the system tries if the C(preferred_lb_method)
      is unsuccessful in picking a pool.
    type: str
  ...
  fallback_ip:
    description:
    - Specifies the IPv4, or IPv6 address of the server to which the system directs
      requests when it cannot use one of its pools to do so. Note that the system
      uses the fallback IP only if you select the C(fallback_ip) load balancing method.
    type: str
  fallback_lb_method:
    choices:
    - round-robin
    ...
    - none
    description:
    - The load balancing mode that the system tries if both the C(preferred_lb_method)
      and C(alternate_lb_method)s are unsuccessful in picking a pool.
    type: str
  max_answers_returned:
    description:
    - Specifies the maximum number of available virtual servers that the system lists
      in a response.
    - The maximum is 500.
    type: int
    version_added: 2.8
  members:
    ...
    - The order of the members in this list is the order that they will be listed
      in the pool.
    suboptions:
      server:
        description:
  ...
  state:
    ...
    default: present
    description:
    - Pool state. When C(present), ensures that the pool is created and enabled. When
      C(absent), ensures that the pool is removed from the system. When C(enabled)
      or C(disabled), ensures that the pool is enabled or disabled (respectively)
  ...
  type:
    ...
    - srv
    description:
    - The type of GTM pool that you want to create. On BIG-IP releases prior to version
      12, this parameter is not required. On later versions of BIG-IP, this is a required
      parameter.
    type: str
