...
notes:
- In previous versions of this module, which used the SDK, the C(name) parameter would
  act as C(fqdn) if C(address) or C(fqdn) were not provided.
options:
  address:
    aliases:
    - ip
  ...
  fqdn:
    ...
    - FQDN name of the pool member. This can be any name that is a valid RFC 1123
      DNS name. Therefore, the only characters that can be used are "A" to "Z", "a"
      to "z", "0" to "9", the hyphen ("-") and the period (".").
  ...
  name:
    ...
    - This parameter is optional and, if not specified, a node name will be created
      automatically from either the specified C(address) or C(fqdn).
  ...
  rate_limit:
    description:
    - Pool member rate limit (connections-per-second). Setting this to 0 disables
      the limit.
    type: int
  ratio:
    description:
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.common import is_valid_hostname
