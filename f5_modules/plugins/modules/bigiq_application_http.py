...
- Tim Rupp (@caphrim007)
description:
- Manages BIG-IQ applications used for load balancing an HTTP application on port
  80 on BIG-IP.
...
options:
  ...
  service_environment:
    ...
    - The service environment type will be discovered by this module automatically.
      Therefore, it is crucial that you maintain unique names for items in the different
      service environment types (at this time, SSGs and BIGIPs).
    type: str
  state:
    choices:
    - absent
