...
- Tim Rupp (@caphrim007)
description:
- Manages BIG-IQ applications used for load balancing an HTTPS application on port
  443 with a Web Application Firewall (WAF) using an ASM Rapid Deployment policy.
...
options:
  ...
  client_ssl_profile:
    ...
    suboptions:
      cert_key_chain:
        ...
        - This option is always a list. The keys in the list dictate the details of
          the client/key/chain/passphrase combination.
        - Note that BIG-IPs can only have one of each type of each certificate/key
          ...
          per profile.
        - If you attempt to assign two RSA, DSA, or ECDSA certificate/key combo, the
          device will reject this.
        - This list is a complex list that specifies a number of keys.
        - When creating a new profile, if this parameter is not specified, the default
          value of C(inherit) will be used.
        suboptions:
          cert:
            description:
  ...
  inbound_virtual:
    ...
    - Traffic destined to the C(redirect_virtual) will be offloaded to this parameter
      to ensure that proper redirection from insecure, to secure, occurs.
    suboptions:
      address:
        description:
  ...
  redirect_virtual:
    ...
    - Traffic destined to this parameter will be offloaded to the C(inbound_virtual)
  ...
  service_environment:
    description:
    - Specifies the name of service environment that the application will be deployed
      to.
    - When creating a new application, this parameter is required.
    type: str
  state:
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.icontrol import bigiq_version
