...
options:
  local_ip:
    description:
    - Specifies the local IP address of the system that is logging. To provide no
      local IP, specify the value C(none).
    - When creating a remote syslog, if this parameter is not specified, the default
      value C(none) is used.
    type: str
  name:
    description:
    - Specifies the name of the syslog object.
    - This option is required when multiple C(remote_host) with the same IP or hostname
  ...
  remote_host:
    description:
    - Specifies the IP address, or hostname, for the remote system to which the system
      sends log messages.
    required: true
    type: str
  remote_port:
    description:
    - Specifies the port that the system uses to send messages to the remote logging
      server.
    - When creating a remote syslog, if this parameter is not specified, the default
      value C(514) is used.
    type: str
