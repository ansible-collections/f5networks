...
options:
  auto_init:
    description:
    - If C(yes), the BIGIP will automatically create outbound connections to the active
      pool members in the specified C(pool) using the configuration of the specified
      C(transport_config).
    - For auto-initialization to attempt to create a connection, the peer must be
      included in a route that is attached to a router instance. For each router instance
      that the peer is contained in, a connection will be initiated.
    - The C(auto_init) logic will verify at C(auto_init_interval) if the a connection
      exists between the BIG-IP and the pool members of the pool. If a connection
      does not exist, it will attempt to reestablish one.
    type: bool
  auto_init_interval:
    description:
    - Specifies the interval that attempts to initiate a connection occur.
    - The default value upon peer object creation, that supplied by the system is
      C(5000) milliseconds.
  ...
  ratio:
    description:
    - Specifies the ratio to be used for selection of a peer within a list of peers
      in a ltm route.
    - The accepted range is between 0 and 4294967295 inclusive.
    type: int
  state:
  ...
  transport_config:
    description:
    - The name of the ltm virtual or ltm transport-config to use for creating an outgoing
      connection.
