...
options:
  always_send:
    description:
    - Send the cookie persistence entry on every reply, even if the entry has previously
  ...
  cookie_encryption:
    ...
    - When C(preferred), generate an encrypted cookie, but accepts both encrypted
  ...
  cookie_method:
    ...
    - When C(hash), specifies that the server provides the cookie, which the system
      then maps consistently to a specific node. This persistence type requires a
      C(cookie_name) value.
    - When C(insert), specifies that the system inserts server information, in the
      form of a cookie, into the header of the server response.
    - When C(passive), specifies that the server provides the cookie, formatted with
      the correct server information and timeout. This persistence type requires a
      C(cookie_name) value.
    - When C(rewrite), specifies that the system intercepts the BIGipCookie header,
      sent from the server, and overwrites the name and value of that cookie.
    type: str
  cookie_name:
    description:
  ...
  encrypt_cookie_pool_name:
    description:
    - Specifies whether the pool-name in the inserted BIG-IP default cookie should
      be encrypted.
    type: bool
  ...
  expiration:
    description:
    - Specifies the expiration time of the cookie. By default the system generates
      and uses session cookie. This cookie expires when the user session expires,
      that is when the browser is closed.
    suboptions:
      days:
        description:
        - Cookie expiration time in days, the value must be in range from C(0) to
          C(24855) days.
        type: int
      hours:
        description:
        - Cookie expiration time in hours, the value must be in the range from C(0)
          to C(23) hours.
        type: int
      minutes:
        description:
  ...
  http_only:
    description:
    - Specifies whether the httponly attribute should be enabled or disabled for the
      inserted cookies.
    type: bool
  match_across_pools:
    description:
  ...
  match_across_services:
    description:
    - When C(yes), specifies that all persistent connections from a client IP address
      that go to the same virtual IP address also go to the same node.
    - When creating a new profile, if this parameter is not specified, the default
      is provided by the parent profile.
    type: bool
  match_across_virtuals:
    description:
    - When C(yes), specifies that all persistent connections from the same client
      IP address go to the same node.
    - When creating a new profile, if this parameter is not specified, the default
      is provided by the parent profile.
    type: bool
  name:
  ...
  override_connection_limit:
    description:
    - When C(yes), specifies that the system allows you to specify that pool member
      connection limits will be overridden for persisted clients.
  ...
  secure:
    description:
    - Specifies whether the secure attribute should be enabled or disabled for the
      inserted cookies.
    type: bool
  state:
    choices:
  ...
  update_password:
    ...
    default: always
    description:
    - C(always) will allow to update passphrases if the user chooses to do so. C(on_create)
