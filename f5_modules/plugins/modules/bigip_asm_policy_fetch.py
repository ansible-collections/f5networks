...
options:
  ...
  inline:
    description:
    - If C(yes), the ASM policy will be exported C(inline) as a string instead of
      a file.
    - The policy can be be retrieved in playbook C(result) dictionary under C(inline_policy)
      key.
    type: bool
  name:
    description:
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.icontrol import download_asm_file
