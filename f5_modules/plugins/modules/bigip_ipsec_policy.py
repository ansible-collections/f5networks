...
options:
  ...
  ipcomp:
    ...
    - When C(deflate), specifies that IPComp is enabled and uses the Deflate compression
      algorithm.
    type: str
  ipv4_interface:
    description:
    - When C(mode) is C(interface) indicate if the IPv4 C(any) address should be used.
      By default C(BIG-IP) assumes C(any6) address for tunnel addresses when C(mode)
      is C(interface).
  ...
  mode:
    ...
    - When C(tunnel), specifies a mode that includes encapsulation of the header as
      ...
      trailer, and authentication tag). If you select this option, you must also provide
  ...
  protocol:
    ...
    - Options include ESP (Encapsulating Security Protocol) or AH (Authentication
      Header).
    type: str
  route_domain:
    description:
