...
options:
  geolocations:
    ...
    suboptions:
      blacklist:
        description:
        - A list of countries to be put on blacklist, must not have overlapping elements
          with C(whitelist).
        type: list
      whitelist:
        description:
        - A list of countries to be put on whitelist, must not have overlapping elements
          with C(blacklist).
        type: list
    type: dict
  heavy_urls:
    ...
    suboptions:
      ...
      include:
        ...
        suboptions:
          ...
          url:
            description:
            - Specifies the URL to be added to the list of heavy URLs, in addition
  ...
  mobile_detection:
    ...
    - Configures detection of mobile applications built with the Anti-Bot Mobile SDK
      and defines how requests from these mobile application clients are handled.
    suboptions:
      ...
      client_side_challenge_mode:
        ...
        - cshui
        description:
        - Action to take when a CAPTCHA or Client Side Integrity challenge needs to
          be presented.
        ...
        - When C(cshui) the SDK checks for human interactions with the screen in the
          last few seconds. If none are detected, the traffic is blocked.
        type: str
      enabled:
        description:
        - When C(yes), requests from mobile applications built with Anti-Bot Mobile
          SDK will be detected and handled according to the parameters set.
        - When C(no), these requests will be handled like any other request which
          may let attacks in, or cause false positives.
        type: bool
  ...
  rtbh_enable:
    description:
    - Specifies whether to enable Remote Triggered Black Hole C(RTBH) of attacking
      IPs by advertising BGP routes.
    type: bool
  scrubbing_duration:
    description:
    - Specifies the duration of the Traffic Scrubbing BGP route advertisement, in
      seconds.
    - The accepted range is between 0 and 4294967295 inclusive.
    type: int
  scrubbing_enable:
    description:
    - Specifies whether to enable Traffic Scrubbing during attacks by advertising
      BGP routes.
    type: bool
