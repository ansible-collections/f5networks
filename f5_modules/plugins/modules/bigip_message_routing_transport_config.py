...
options:
  ...
  profiles:
    description:
    - Specifies a list profiles for the outgoing connection to use to direct and manage
      traffic.
    type: list
  rules:
    description:
    - The iRules you want run on this transport config. iRules help automate the intercepting,
      processing, and routing of application traffic.
    type: list
  src_addr_translation:
    description:
    - Specifies the type of source address translation enabled for the transport config
      and the pool that the source address translation will use.
    suboptions:
      pool:
        description:
        - Specifies the name of a LSN or SNAT pool used by the specified transport
          config.
        - 'Name can also be specified in C(fullPath) format: C(/Common/foobar)'
        - When C(type) is C(none) or C(automap) the pool parameter will be replaced
          by C(none) keyword, thus any defined C(pool) parameter will be ignored.
        type: str
      type:
        choices:
        - snat
        ...
        - Specifies the type of source address translation associated with the specified
          transport config.
        - When set to C(snat) the C(pool) parameter needs to contain a name for a
          valid LSN or SNAT pool.
        type: str
    type: dict
  src_port:
    description:
    - Specifies the source port to be used for the connection being created.
    - If no value is specified an ephemeral port is chosen for the connection being
      created.
