...
- Tim Rupp (@caphrim007)
description:
- Manages a BIG-IP configuration by allowing TMSH commands that modify running configuration,
  or merge SCF formatted files into the running configuration. Additionally, this
  module is of significant importance because it allows you to save your running configuration
  to disk. Since the F5 module only manipulate running configuration, it is important
  that you utilize this module to save that running config.
...
options:
  merge_content:
    ...
    - Loads the specified configuration that you want to merge into the running configuration.
      ...
      merge).
    - If you need to read configuration from a file or template, use Ansible's C(file)
      or C(template) lookup plugins respectively.
    type: str
  reset:
    default: false
    description:
    - Loads the default configuration on the device.
    - If this option is specified, the default configuration will be loaded before
  ...
  save:
    ...
    - This operation is performed after any changes are made to the current running
      config. If no changes are made, the configuration is still saved to the startup
      config.
  ...
  verify:
    default: false
    description:
    - Validates the specified configuration to see whether they are valid to replace
