...
options:
  accept_eula:
    default: false
    description:
    - Declares whether you accept the BIG-IP EULA or not. By default, this value is
      C(no). You must specifically declare that you have viewed and accepted the license.
      This module will not present you with that EULA though, so it is incumbent on
      you to read it.
    - The EULA can be found here; https://support.f5.com/csp/article/K12902.
    - This parameter is not required when C(state) is C(absent) and will be ignored
      if it is provided.
    type: bool
  license_key:
    description:
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.icontrol import iControlRestSession
