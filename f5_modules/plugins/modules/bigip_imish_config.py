...
- Tim Rupp (@caphrim007)
description:
- This module provides an implementation for working with advanced routing configuration
...
options:
  after:
    ...
    - The ordered set of commands to append to the end of the command stack if a change
      needs to be made.
    - Just like with I(before) this allows the playbook designer to append a set of
      commands to be executed after the command set.
    type: list
  backup:
    default: 'no'
    description:
  ...
  diff_ignore_lines:
    ...
    - Use this argument to specify one or more lines that should be ignored during
      the diff.
    - This is used for lines in the configuration that are automatically updated by
      the system.
  ...
  intended_config:
    ...
    - The C(intended_config) provides the master configuration that the node should
      conform to and is used to check the final running-config against.
    ...
    - When specifying this argument, the task should also modify the C(diff_against)
      value and set it to I(intended).
    type: str
  lines:
    aliases:
    - commands
  ...
  match:
    ...
    - Instructs the module on the way to perform the matching of the set of commands
      against the current device config.
    ...
    - Finally, if match is set to I(none), the module will not attempt to compare
  ...
  parents:
    ...
    - The ordered set of parents that uniquely identify the section or hierarchy the
      commands should be checked against.
    - If the C(parents) argument is omitted, the commands are checked against the
      set of top level or global commands.
    type: list
  replace:
    choices:
    - line
    ...
    - If the replace argument is set to I(line) then the modified lines are pushed
      to the device in configuration mode.
    - If the replace argument is set to I(block) then the entire command block is
      pushed to the device in configuration mode if any line is not correct.
    type: str
  route_domain:
    default: 0
    description:
  ...
  save_when:
    ...
    - When changes are made to the device running-configuration, the changes are not
      copied to non-volatile storage by default.
    - If the argument is set to I(always), then the running-config will always be
      copied to the startup-config and the I(modified) flag will always be set to
      C(True).
    - If the argument is set to I(modified), then the running-config will only be
      copied to the startup-config if it has changed since the last save to startup-config.
    - If the argument is set to I(never), the running-config will never be copied
      to the startup-config.
    - If the argument is set to I(changed), then the running-config will only be copied
      to the startup-config if the task has made a change.
    type: str
  src:
    description:
...
from ansible_collections.f5networks.network.plugins.module_utils.network.common.config import NetworkConfig, dumps
from ansible_collections.f5networks.network.plugins.module_utils.network.common.utils import to_list
