...
options:
  acl_order:
    ...
    - The lower number the higher the ACL will be in the general order, with lowest
      number C(0) being the topmost one.
  ...
  entries:
    ...
    - The order in which the rules are placed as arguments to this parameter, determines
      their order in the ACL, in other words changing the order of the same elements
    ...
    suboptions:
      action:
        ...
        - continue
        description:
        - Specifies the action that the access control entry takes when a match for
          this access control entry is encountered.
        required: true
        type: str
      dst_addr:
        description:
        - Specifies the destination IP address for the access control entry.
        - When set to C(any) the ACL will match any destination address, C(dst_mask)
      ...
      dst_mask:
        ...
        - Optional parameter that specifies the destination network mask for the access
          control entry.
        - If not specified and C(dst_addr) is not C(any) the C(dst_addr) is deemed
          to be host address.
        type: str
      dst_port:
        description:
      ...
      dst_port_range:
        ...
        - Parameter is mutually exclusive with C(dst_port_range).
        - To indicate all ports the C(dst_port) parameter must be used and set to
          C(*).
        type: str
      host_name:
        description:
      ...
      protocol:
        ...
        - 'Specifies the protocol: C(tcp), C(udp), C(icmp) or C(all) protocols, to
          which the access control entry applies.'
        type: str
      scheme:
        choices:
        - http
        ...
        description:
        - This parameter applies to Layer 7 access control entries only.
        - 'Specifies the URI scheme: C(http), C(https) or C(any) on which the access
          control entry operates.'
        type: str
      src_addr:
        description:
        - Specifies the source IP address for the access control entry.
        - When set to C(any) the ACL will match any source address, C(src_mask) is
          ignored in this case.
        type: str
      src_mask:
        description:
        - Optional parameter that specifies the source network mask for the access
          control entry.
        - If not specified and C(src_addr) is not C(any) the C(src_addr) is deemed
          to be host address.
        type: str
      src_port:
        description:
      ...
      src_port_range:
        ...
        - Parameter is mutually exclusive with C(src_port_range).
        - To indicate all ports the C(src_port) parameter must be used and set to
          C(*).
        type: str
    type: list
  name:
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.ipaddress import is_valid_ip_network
