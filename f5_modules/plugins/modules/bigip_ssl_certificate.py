...
description:
- This module will import/delete SSL certificates on BIG-IP LTM. Certificates can
  be imported from certificate and key files on the local disk, in PEM format.
...
options:
  content:
    ...
    - cert_content
    description:
    - Sets the contents of a certificate directly to the specified value. This is
      used with lookup plugins or for anything with formatting or
  ...
  state:
    ...
    default: present
    description:
    - Certificate state. This determines if the provided certificate and key is to
      be made C(present) on the device or C(absent).
    type: str
requirements:
- BIG-IP >= v12
