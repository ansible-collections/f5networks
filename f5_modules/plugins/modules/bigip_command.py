...
description:
- Sends a TMSH or BASH command to an BIG-IP node and returns the results read from
  ...
  met.
- This module is B(not) idempotent, nor will it ever be. It is intended as a stop-gap
  measure to satisfy automation requirements until such a time as a real module has
  been developed to configure in the way you need.
- If you are using this module, you should probably also be filing an issue to have
  a B(real) module created for your needs.
...
options:
  ...
  commands:
    ...
    - Only C(tmsh) commands are supported. If you are piping or adding additional
      logic that is outside of C(tmsh) (such as grep'ing, awk'ing or other shell related
      things that are not C(tmsh), this behavior is not supported.
    required: true
    type: raw
  interval:
    default: 1
  ...
  match:
    ...
    - The I(match) argument is used in conjunction with the I(wait_for) argument to
      ...
      set to C(all) then all conditionals in the I(wait_for) must be satisfied. If
  ...
  transport:
    ...
    - Configures the transport connection to use when connecting to the remote device.
      The transport argument supports connectivity to the device over cli (ssh) or
      rest.
    required: true
    version_added: 2.5
  wait_for:
...
from ansible_collections.f5networks.network.plugins.module_utils.network.common.parsing import FailedConditionsError
from ansible_collections.f5networks.network.plugins.module_utils.network.common.parsing import Conditional
from ansible_collections.f5networks.network.plugins.module_utils.network.common.utils import ComplexList
...
try:
    ...
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.common import is_cli

try:
    from ansible_collections.f5networks.f5_modules.plugins.module_utils.network.f5.common import run_commands
