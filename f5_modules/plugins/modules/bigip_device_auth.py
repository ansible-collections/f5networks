...
- Manage the system authentication configuration. This module can assist in configuring
  a number of different system authentication types. Note that this module can not
...
options:
  authentication:
    ...
    - When C(use-first-server), specifies that the system sends authentication attempts
      to only the first server in the list.
    - When C(use-all-servers), specifies that the system sends an authentication request
      to each server until authentication succeeds, or until the system has sent a
  ...
  servers:
    description:
    - Specifies a list of the IPv4 addresses for servers using the Terminal Access
      Controller Access System (TACACS)+ protocol with which the system communicates
      to obtain authorization data.
    - For each address, an alternate TCP port number may be optionally specified by
      specifying the C(port) key.
  ...
  service_name:
    ...
    - firewall
    description:
    - Specifies the name of the service that the user is requesting to be authorized
      to use.
    - Identifying what the user is asking to be authorized for, enables the TACACS+
      server to behave differently for different types of authorization requests.
  ...
  state:
    ...
    - When C(present), guarantees that the system is configured for the specified
      C(type).
  ...
  type:
    ...
    - This module only supports a subset, at this time, of the total available auth
      types.
    type: str
  update_secret:
    choices:
    ...
    - C(on_create) will only set the secret when a C(use_auth_source) is C(yes) and
      TACACS+ is not currently the auth source.
    type: str
  use_for_auth:
    description:
