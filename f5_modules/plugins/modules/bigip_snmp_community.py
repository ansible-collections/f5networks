...
- Assists in managing SNMP communities on a BIG-IP. Different SNMP versions are supported
  by this module. Take note of the different parameters offered by this module, as
  different parameters work for different versions of SNMP. Typically this becomes
  an interest if you are mixing versions C(v2c) and C(3).
...
options:
  access:
    ...
    - When creating a new community, if this parameter is not specified, the default
      is C(ro).
    - When C(ro), specifies that the user can view the MIB, but cannot modify the
      MIB.
  ...
  ip_version:
    ...
    - When creating a new community, if this value is not specified, the default of
      C(4) will be used.
    - This parameter is only relevant when C(version) is C(v1), or C(v2c). If C(version)
      is something else, this parameter is ignored.
    type: str
  name:
    ...
    - The name C(public) is a reserved name on the BIG-IP. This module handles that
      name differently than others. Functionally, you should not see a difference
      however.
    type: str
  oid:
    description:
    ...
    - When C(version) is either C(v1) or C(v2c), if this value is specified, then
      C(source) must not be set to C(all).
    type: str
  partition:
    default: Common
    description:
  ...
  snmp_auth_protocol:
    ...
    - When C(md5), specifies that the system uses the MD5 algorithm to authenticate
      the user.
    - When C(sha), specifies that the secure hash algorithm (SHA) to authenticate
      the user.
    - When C(none), specifies that user does not require authentication.
    - When creating a new SNMP C(v3) community, if this parameter is not specified,
      the default of C(sha) will be used.
    type: str
  ...
  snmp_privacy_protocol:
    ...
    - When creating a new SNMP C(v3) community, if this parameter is not specified,
      the default of C(aes) will be used.
    type: str
  snmp_username:
    description:
    - Specifies the name of the user for whom you want to grant access to the SNMP
      v3 MIB.
  ...
  source:
    ...
    - This parameter is only relevant when C(version) is C(v1), or C(v2c). If C(version)
      is something else, this parameter is ignored.
    - If C(source) is set to C(all), then it is not possible to specify an C(oid).
      This will raise an error.
    - This parameter should be provided when C(state) is C(absent), so that the correct
      community is removed. To remove the C(public) SNMP community that comes with
      a BIG-IP, this parameter should be set to C(default).
    type: str
  state:
    choices:
    - present
  ...
  update_password:
    ...
    - C(always) will allow to update passwords if the user chooses to do so. C(on_create)
      will only set the password for newly created resources.
    type: str
  version:
    choices:
    - v1
    ...
    default: v2c
    description:
    - Specifies to which Simple Network Management Protocol (SNMP) version the trap
      destination applies.
    type: str
