...
options:
  client_timeout:
    ...
    - This is the time limit for the client to provide the application data required
      to select a back-end server. That is, this is the maximum time that the BIG-IP
      system waits for information about the sender and the target.
    - This information typically arrives at the beginning of the FIX logon packet.
    - When C(0), or C(immediate), allows for no time beyond the moment of the first
      packet transmission.
  ...
  explicit_flow_migration:
    ...
    - When C(yes), this parameter stops automatic migration to ePVA, and requires
      that the iRule explicitly trigger ePVA processing by invoking the C(release_flow)
      iRule command. This allows an iRule author to control when the connection uses
      the ePVA hardware.
    type: bool
  idle_timeout:
    description:
    ...
    - When C(0), or C(immediate), specifies that the system deletes connections immediately
  ...
  ip_df_mode:
    ...
    - When C(preserve), sets the outgoing Packet's IP Header DF bit to be same as
      incoming IP Header DF bit.
    - When C(set), sets the outgoing packet's IP Header DF bit.
    - When C(clear), clears the outgoing packet's IP Header DF bit.
    type: str
  ip_tos_to_client:
    description:
    - Specifies, for IP traffic passing through the system to clients, whether the
      system modifies the IP type-of-service (ToS) setting in an IP packet header.
    - May be a number between 0 and 255 (inclusive). When a number, specifies the
      IP ToS setting that the system inserts in the IP packet header.
    - When C(pass-through), specifies that the IP ToS setting remains unchanged.
    - When C(mimic), specifies that the system sets the ToS level of outgoing packets
      to the same ToS level of the most-recently received incoming packet.
    type: str
  ip_tos_to_server:
    description:
  ...
  late_binding:
    description:
    - Enables intelligent selection of a back-end server or pool, using an iRule to
      make the selection.
    type: bool
  link_qos_to_client:
    description:
    - Specifies, for IP traffic passing through the system to clients, whether the
      system modifies the link quality-of-service (QoS) setting in an IP packet header.
    ...
    - When a number, specifies the link QoS setting that the system inserts in the
      IP packet header.
  ...
  link_qos_to_server:
    description:
    - Specifies, for IP traffic passing through the system to back-end servers, whether
      the system modifies the link quality-of-service (QoS) setting in an IP packet
      header.
    - The link QoS value prioritizes the IP packet relative to other Layer 2 traffic.
    - You can specify a number between 0 (lowest priority) and 7 (highest priority).
    - When a number, specifies the link QoS setting that the system inserts in the
  ...
  loose_close:
    ...
    - When C(yes), specifies, that the system closes a loosely-initiated connection
      when the system receives the first FIN packet from either the client or the
      server.
    type: bool
  loose_initialization:
    description:
    - When C(yes), specifies that the system initializes a connection when it receives
      any TCP packet, rather that requiring a SYN packet for connection initiation.
    type: bool
  mss_override:
    description:
  ...
  server_sack:
    description:
    - Specifies whether the BIG-IP system processes Selective ACK (Sack) packets in
  ...
  tcp_generate_isn:
    description:
    - When C(yes), specifies that the system generates initial sequence numbers for
      SYN packets, according to RFC 1948.
    type: bool
  tcp_handshake_timeout:
    ...
    - Specifies the acceptable duration for a TCP handshake, that is, the maximum
      ...
      (ACK). If the TCP handshake takes longer than the timeout, the system automatically
      closes the connection.
    - When a number, specifies how long the system can try to establish a TCP handshake
      before timing out.
    - When C(disabled), specifies that the system does not apply a timeout to a TCP
      handshake.
    - When C(indefinite), specifies that attempting a TCP handshake never times out.
    type: str
  tcp_strip_sack:
    description:
    - When C(yes), specifies that the system blocks a TCP selective ACK SackOK option
      from passing to the server on an initiating SYN.
    type: bool
  tcp_time_wait_timeout:
    description:
    - Specifies the number of milliseconds that a connection is in the TIME-WAIT state
      before closing.
    type: int
  tcp_timestamp_mode:
    choices:
  ...
  timeout_recovery:
    ...
    - When C(fallback), reverts the connection to normal FastL4 load-balancing, based
      on the client's TCP header. This causes the BIG-IP system to choose a back-end
      server based only on the source address and port.
    type: str
